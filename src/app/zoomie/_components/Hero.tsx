...
export function Hero() {
  return (
    <section className="flex flex-col items-center py-10 text-center lg:py-16">
      ...
      <h1 className="mt-8 text-[13vw] font-black leading-[0.88] tracking-[-0.04em] text-white sm:text-[64px] lg:text-[76px]">
        Close
        <br />
        your dog&apos;s
        <br />
        <span style={{ color: RING.paws.color }}>rings.</span>
      </h1>
        ...
        {(Object.keys(RING) as RingKey[]).map((key) => (
          <div key={key}>
            ...
            >
              ...
              <span className="text-white/25">/{DAY[key].target}</span>
            </p>
            <p className="mt-1.5 text-[10px] font-semibold uppercase tracking-wider text-white/35">
              {RING[key].label}
            </p>
          </div>
        ))}
